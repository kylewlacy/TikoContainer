...
namespace UnitTests
{
	...
	public sealed class TikoContainerTest
	{
		...
		{
			var result = TikoContainer.Resolve<ITestAttributeClass> ();
			var result1 = TikoContainer.Resolve<ITestAttributeClass> ();
			Assert.IsNotNull (result);
			Assert.IsNotNull (result1);
			Assert.AreEqual (result, result1);
		}
	...
	public interface ITestAttributeClass
	{
		string Test ();
	}

	[Resolves (typeof(TestAttributeClass))]
	public sealed class TestAttributeClass : ITestAttributeClass
	{
		public string Test ()
		{
			return "Test attribute class";
		}
	}
}
